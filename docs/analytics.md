...
- `SiteVisit`: the first mount of `PublicLayout` in a loaded browser page. A module-level guard prevents duplicates from StrictMode, rerenders, navigation, or repeated mounting. A full reload/new tab can count again. Admin-only routes do not record a site opening. Public activity is also excluded when the request carries a valid JWT with the `Admin` role.
...
Public tracking uses a small fire-and-forget fetch with `keepalive: true` and `credentials: 'omit'`. It attaches the existing admin JWT when available so backend authentication can recognize and ignore administrative events. It deliberately bypasses the shared Axios response interceptor so an analytics failure cannot log out the admin. Both synchronous and asynchronous failures are suppressed; events are not retried or queued. Missing browser crypto, offline mode, blockers, or server failures can cause undercounting without breaking the site. Keepalive supports navigation-time dispatch but does not guarantee delivery.

The public endpoint remains anonymous and returns `204 No Content` for both recorded events and authenticated administrator events. The backend is authoritative: after optional JWT authentication, it checks the `Admin` role and skips validation and persistence for that request. Anonymous requests and authenticated users without that role are processed normally. An administrator browsing while logged out, with an expired/invalid token, in a separate incognito session, or from another browser without the token is indistinguishable from an anonymous visitor and can still be counted. Existing rows are not changed or reclassified.

Only `Id`, `SessionId`, `EventType`, `DocumentId`, and `CreatedAt` are persisted. No IP address, user agent, name, email, account ID, page URL, or cookies are stored by this feature. Hosting/provider access logs are independent of this application table. The persistent random browser identifier is not an identified student account, and this is not a bot-proof analytics system.
